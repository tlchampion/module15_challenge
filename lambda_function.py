...
def validate_data(age, investment_amount, intent_request):
    ...
    # Validate that the user is over zero and under 65 years old
    if age is not None:
        age = parse_int(age)
        if age not in range(0,65):
            return build_validation_result(
                False,
                "age",
                "Age should be larger than zero and lower than 65 years.",
            )
...
"""
def get_portfolio_by_risk(risk_level):
    if risk_level == "none":
...
def recommend_portfolio(intent_request):
    ...
    # code is a modified version of the lambda_function.py file found in the module 15 in class activities.
    ...
    return close(
        ...
        "Fulfilled",
        {
            ...
            """.format(
                get_portfolio_by_risk(risk_level)
            ),
        },
    )
